...
public class TestGetandPostWithSSL {
	...
	public void testDeleteAndPut() {
		...
		RestResponse response = HttpsClientHelper
				.performPostWithSSL("https://localhost:8443/laptop-bag/webapi/sslres/add", jsonBody, ContentType.APPLICATION_JSON, headers);
		...
		response= HttpsClientHelper
				.performPutWithSSL("https://localhost:8443/laptop-bag/webapi/sslres/update", jsonPutBody, ContentType.APPLICATION_JSON, headers);
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode() );
		...
		response = HttpsClientHelper
				.performDeleteRequestWithSSL("https://localhost:8443/laptop-bag/webapi/sslres/delete/" + id, null);
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
